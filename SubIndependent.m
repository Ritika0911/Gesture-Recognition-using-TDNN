...
TC = [ones([1 3*length(c_11)]) -ones([1 3*length(t_11)]) -ones([1 3*length(h_11)]) -ones([1 3*length(v_11)])...
      ...
      ones([1 3*length(c_15)]) -ones([1 3*length(t_14)]) -ones([1 3*length(h_15)]) -ones([1 3*length(v_15)])];
...
%CIRCLE
net_c = timedelaynet(delay,hidden_layer,'trainbr');
...
%TRIANGLE
net_t = timedelaynet(delay,hidden_layer,'trainbr');
...
%LEFT-RIGHT
net_h = timedelaynet(delay,hidden_layer,'trainbr');
...
%UP-DOWN
net_v = timedelaynet(delay,hidden_layer,'trainbr');
...
H_1 = zeros([4 4 5]);
for i = 1:5
    display(i,'Network Initilization');
    %CIRCLE
    net_c = init(net_c);
